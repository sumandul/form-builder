...
import {
  timeFilterList,
  observedPrecipitationSourceList as sourceList,
} from '@Constants/analyticsConstants';
import { IAnalyticsChartComponentProps } from '../types';

const ObservedPrecipitationChart = ({
  municipalityCode,
  id,
  ...
  const { data: weatherApiCurrentPrecipitation } = useQuery({
    ...
    queryFn: () =>
      getPrecipitationChartData({
        ...
        municipality_code: municipalityCode || selectedMunicipality,
  ...
  //   queryKey: ['get-openweather-current', selectedMunicipality, selectedDay],
  //   enabled: !!selectedMunicipality,
  //   queryFn: () =>
  //     getPrecipitationChartData({
  ...
  const { data: hiwatPrecipitaionData } = useQuery({
    queryKey: [
      'get-hiwat-current-precipitation',
      municipalityCode || selectedMunicipality,
      selectedDay,
      customValue,
    ],
    ...
    select: res => {
      return res.data.data?.map((hiwatData: any) => {
        ...
        return {
          precipitation: hiwatData.today.precipitation,
          date: formattedDate,
        };
      });
    },
  ...
  useEffect(() => {
    if (!weatherApiCurrentPrecipitation || !hiwatPrecipitaionData || !windyData)
      return;
    const datesArray = [
    ...
    const collectiveChartData = uniqueDate.map(date => {
      ...
      const hiwatPrecipitaion = hiwatPrecipitaionData?.find(
        (precip: Record<string, any>) => {
          return precip.date === date;
        },
      )?.precipitation;
      return {
        name: 'A',
        weatherapi_current: weatherapiCurrent
          ...
          : weatherapiCurrent,
        hiwat_current: hiwatPrecipitaion
          ? roundToNthDecimal(Number(hiwatPrecipitaion), 1)
          : hiwatPrecipitaion,
  ...
  }, [hiwatPrecipitaionData, weatherApiCurrentPrecipitation, windyData]);
...
export default ObservedPrecipitationChart;
